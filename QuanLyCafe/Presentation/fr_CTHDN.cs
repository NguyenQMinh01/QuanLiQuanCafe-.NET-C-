    ...
    public partial class fr_CTHDN : Form
    {
        ...
        public void khoitaoluoi()
        {
            try
            {
                ...
                msds.Columns[0].HeaderText = "Số CTHDN";
                msds.Columns[0].Frozen = true;
                msds.Columns[0].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                msds.Columns[0].Width = 100;
                msds.Columns[1].HeaderText = "Mã HDN";
                msds.Columns[1].Width = 100;
                msds.Columns[2].HeaderText = "Mã NL";
                msds.Columns[2].Width = 80;
                msds.Columns[3].HeaderText = "Thành Tiền";
        ...
        public void hienthi()
        {
            string sql = "select macthdn, mahdn,MaNL, FORMAT(thanhtien, '#,0.00' ) as thanhtien ,khuyenmai,soluong from tb_CTHDN  where tb_CTHDN.mahdn = " + SOHDN + "";
        ...
        private void btmoi_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                un_locktext();
                setnull();
                cbhd.Text = (int.Parse(thucthi.laymatop1CTHDN(txtdg.Text))+1).ToString();
                //MessageBox.Show(cbhd.Text);
                string sql = "SELECT macthdn,mahdn, MaNL, thanhtien, khuyenmai, soluong FROM tb_CTHDN where mahdn='" + sohdn + "'";
                msds.DataSource = cn.taobang(sql);
        ...
        private void btluu_Click(object sender, EventArgs e)
        {
            if (cbhang.Text != "")
            {
                ...
                string upsl = "UPDATE tb_CTHDN SET soluong = '" + txtsl.Text + "' where MaNL='" + cbhang.Text + "'";
                string upsl1 = "UPDATE tb_CTHDN SET thanhtien ='" + txttt.Text + "' where MaNL ='" + cbhang.Text + "'";
                string udkm = "update tb_CTHDN set khuyenmai = " + txtgg.Text + " where MaNL='" + cbhang.Text + "'";
                string uptt = "update tb_HDN set tongtien=(SELECT sum(thanhtien*soluong - soluong* khuyenmai) FROM tb_CTHDN) where mahdn='" + cbhd.Text + "'";
                cn.ExcuteNonQuery(uptt);
                cn.ExcuteNonQuery(udkm);
        ...
        private void msds_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                cbhang.Text = msds.Rows[dong].Cells[2].Value.ToString();
                txtsl.Text = msds.Rows[dong].Cells[5].Value.ToString();
                txtdg.Text = msds.Rows[dong].Cells[1].Value.ToString();
                txtgg.Text = msds.Rows[dong].Cells[4].Value.ToString();
                txttt.Text = string.Format("{0:0,0}", msds.Rows[dong].Cells[3].Value.ToString());
                locktext();
            }
            catch
            {
        ...
        {
            txtdg.Text = sohdn;
            thucthi.loadmacthdn(cbhd);
            thucthi.loadmaNL(cbhang);
            hienthi();
            khoitaoluoi();
            locktext();
        }
        ...
        {
            string mahdn = txtdg.Text;
            //MessageBox.Show(mahdn);

            DataTable cthdn = new E_tb_CTHDN().docThongTinCTHoaDonNhap(mahdn);
            FormChuaBaoCao fr = new FormChuaBaoCao("ChiTietHoaDonNhap", cthdn);
            fr.Show();
